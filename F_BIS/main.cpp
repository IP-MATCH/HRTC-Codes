...
int manlove(Allocation& allo){
	...
	try{
		...
		vector<vector<GRBVar> > isDoctorIAllocatedToHospitalJBefore (allo.nbDoctors);
		vector<vector<bool> > isDoctorIAllocatedToHospitalJBool (allo.nbDoctors + 2 * allo.nbCouples);
		
		vector<vector<GRBVar> > isCoupleIAllocatedToChoiceGroupJNumberKBefore (allo.nbCouples);
		...
		for (int i = 0; i < allo.nbDoctors; i++){
			...
			isDoctorIAllocatedToHospitalJBefore[idxDoc].resize(allo.doctors[i].nbPref);
			...
			for (int j = 0; j<allo.doctors[i].nbPref; j++){
				isDoctorIAllocatedToHospitalJBefore[idxDoc][j] = model.addVar(0, 1, 0, GRB_BINARY);
			}
		}
		...
		for (int i = 0; i < allo.nbCouples; i++){
			...
			isCoupleIAllocatedToChoiceGroupJNumberKBefore[i].resize(allo.couples[i].nbPref);
			...
			for (int j = 0; j<allo.couples[i].nbPref; j++){
				isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j] = model.addVar(0, 1, 0, GRB_BINARY);
...
// **
 if(idxHos + 1  == allo.nbHospitals) objFun -= isDoctorIAllocatedToHospitalJ[idxDoc][idxHos];
// **
				if(j == 0) model.addConstr(isDoctorIAllocatedToHospitalJBefore[i][j] == isDoctorIAllocatedToHospitalJ[idxDoc][idxHos]);
				else model.addConstr(isDoctorIAllocatedToHospitalJBefore[i][j] == isDoctorIAllocatedToHospitalJ[idxDoc][idxHos] + isDoctorIAllocatedToHospitalJBefore[i][j-1]);
...
// **
 if(idxHos1 + 1  == allo.nbHospitals) objFun -= isCoupleIAllocatedToChoiceGroupJNumberK[i][j][k];
 if(idxHos2 + 1  == allo.nbHospitals) objFun -= isCoupleIAllocatedToChoiceGroupJNumberK[i][j][k];
// **
		...
		for (int i = 0; i < allo.nbDoctors; i++){
			for (int j = 0; j<allo.doctors[i].nbPref; j++){
				int idxHos = allo.doctors[i].preferences[j] - 1;
				model.addConstr(allo.hospitals[idxHos].cap * (1 - isDoctorIAllocatedToHospitalJBefore[i][j]) - fillingOfHospitalJUpToRankK[idxHos][allo.doctors[i].ranks[j]-1] <= 0);
			}	
		} 
		...
		for (int i = 0; i < allo.nbCouples; i++){
			...
			for (int j = 0; j<allo.couples[i].nbPref; j++){
				...
				if(j == 0) model.addConstr(isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j] == leftside);
				else model.addConstr(isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j] == leftside + isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j-1]);
				for(int k=0; k<allo.couples[i].preferences1[j].size(); k++){
					...
					if(allo.couples[i].preferences1[j][k] == allo.couples[i].preferences2[j][k]){ 
						...
						for (int l = 0; l < allo.nbCouples; l++){
							if(l != i){
								for (int m = 0; m<allo.couples[l].nbPref; m++){
									for(int n=0; n<allo.couples[l].preferences1[m].size(); n++){
										if(allo.couples[l].preferences1[m][n] - 1 == idxHos && allo.couples[l].preferences2[m][n] - 1 == idxHos && 
											max(allo.couples[l].ranks1[m][n], allo.couples[l].ranks2[m][n]) > idxRanDis + 1 && min(allo.couples[l].ranks1[m][n], allo.couples[l].ranks2[m][n]) <= idxRanDis + 1)
											rhs += isCoupleIAllocatedToChoiceGroupJNumberK[l][m][n];
									}
								}			
							}
						}

						model.addConstr(allo.hospitals[idxHos].cap * ( 1 - isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j]) - (1 - isDoctorIAllocatedToHospitalJ[idxDocFav][idxHos]) <= (fillingOfHospitalJUpToRankK[idxHos][idxRanDis] - isDoctorIAllocatedToHospitalJ[idxDocDis][idxHos] - rhs));
				}
					else{ // If the couple chose different hospital
						int idxHos1 = allo.couples[i].preferences1[j][k] - 1;
						int idxHos2 = allo.couples[i].preferences2[j][k] - 1;
						model.addConstr(allo.hospitals[idxHos1].cap * ( 1 - (isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j]+alpha1[i][j][k])) <= fillingOfHospitalJUpToRankK[idxHos1][allo.couples[i].ranks1[j][k]-1] - isDoctorIAllocatedToHospitalJ[idxDoc1][idxHos1]);
						model.addConstr(allo.hospitals[idxHos2].cap * ( 1 - (isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j]+alpha2[i][j][k])) <= fillingOfHospitalJUpToRankK[idxHos2][allo.couples[i].ranks2[j][k]-1] - isDoctorIAllocatedToHospitalJ[idxDoc2][idxHos2]);
	...
	//	for (int j = 0; j < allo.oldHospitals.size(); j++){
		for (int j = 0; j < allo.oldHospitals.size()-1; j++){
	...
	//	allo.infos.td = 1.0 - double(n1 - allo.oldHospitals.size()) / double(n2 - allo.oldHospitals.size());
		allo.infos.td = 1.0 - double(n1 - allo.oldHospitals.size()-1.0) / double(n2 - allo.oldHospitals.size()-1.0);	
	...
	//	allo.infos.contUB = modelRelaxed.get(GRB_DoubleAttr_ObjVal);
		...
		//	if(allo.assignmentByDoctor[i] == -1){
			if(allo.assignmentByDoctor[i] == -1 || allo.assignmentByDoctor[i] == allo.nbHospitals){
	...
	//	double vtaRD = 0; double vtaRU = 0;
		...
		//	if(allo.assignmentByDoctor[idxDoc1] == -1){
			if(allo.assignmentByDoctor[idxDoc1] == -1 || allo.assignmentByDoctor[idxDoc1] == allo.nbHospitals){
				cout << i << " was not assigned " << endl;
				noAC+=1.0;
			}
		//	if(allo.assignmentByDoctor[idxDoc2] == -1){
			if(allo.assignmentByDoctor[idxDoc2] == -1 || allo.assignmentByDoctor[idxDoc2] == allo.nbHospitals){
				cout << i << " was not assigned " << endl;
				noAC+=1.0;
			}
		}
		...
		allo.infos.perC = noAC / float(allo.nbCouples * 2.0);
		...
		for(int i=0;i<allo.nbHospitals-1;i++){
			if(maxR[i] == -1){
