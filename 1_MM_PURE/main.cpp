...
int manlove(Allocation& allo){
	...
	try{
		...
		for (int i = 0; i < allo.nbCouples; i++){
			...
			//			model.addConstr(allo.hospitals[idxHos].cap * ( 1 - leftside) - 1 + isDoctorIAllocatedToHospitalJ[idxDocFav][idxHos] + isDoctorIAllocatedToHospitalJ[idxDocDis][idxHos] <= fillingOfHospitalJUpToRankK[idxHos][idxRanDis]);
						...
						for(int l = idxRanFav+1; l<= idxRanDis; l++){
							for(int m = 0; m < allo.hospitals[idxHos].preferences[l].size();m++){
								...
								if(idxDocLoc != idxDocDis && idxDocLoc != idxDocFav){
									model.addConstr(allo.hospitals[idxHos].cap * ( 1 - leftside) - 1 + isDoctorIAllocatedToHospitalJ[idxDocLoc][idxHos] + isDoctorIAllocatedToHospitalJ[idxDocDis][idxHos] <= fillingOfHospitalJUpToRankK[idxHos][idxRanDis]);
								}
							}
						}	
		...
		model.getEnv().set(GRB_DoubleParam_OptimalityTol, 0.000000001);
		model.optimize();
