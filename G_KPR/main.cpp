			...
			for (int j = 0; j<allo.couples[i].nbPref; j++){
				...
				for(int k=0; k<allo.couples[i].preferences1[j].size(); k++){
					...
					if(allo.couples[i].preferences1[j][k] == allo.couples[i].preferences2[j][k]){ 
						...
						model.addConstr(allo.hospitals[idxHos].cap * ( 1 - isCoupleIAllocatedToChoiceGroupJNumberKBefore[i][j]) - (1 - isDoctorIAllocatedToHospitalJ[idxDocFav][idxHos]) <= (fillingOfHospitalJUpToRankK[idxHos][idxRanDis] - isDoctorIAllocatedToHospitalJ[idxDocDis][idxHos]));
					}
